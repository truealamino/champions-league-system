...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-[#001C55] via-[#00369f] to-[#001C55] text-white font-sans">
        ...
        <div className="relative z-10 flex flex-col justify-between h-full">
          <div>
            ...
            <nav className="space-y-4">
              <SidebarLink href="/admin" icon={faHouse}>Dashboard</SidebarLink>
              <SidebarLink href="/admin/championships" icon={faTrophy}>Campeonatos</SidebarLink>
              <SidebarLink href="/admin/teams" icon={faUsers}>Times</SidebarLink>
              <SidebarLink href="/admin/players" icon={faShirt}>Jogadores</SidebarLink>
            </nav>
          </div>
      ...
      {/* Main content */}
      <main className="flex-1 p-8 overflow-y-auto bg-gradient-to-br from-[#F8F8F8] to-[#E0E0E0] text-gray-900">
        {children}
      </main>
    </div>
  );
...
function SidebarLink({ href, icon, children }: { href: string; icon: any; children: React.ReactNode }) {
  return (
    <Link href={href}>
